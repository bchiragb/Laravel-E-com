    ...
    public function dataTable(QueryBuilder $query): EloquentDataTable
    {
        return (new EloquentDataTable($query))
            ->addColumn('country', function($query){
                return $query->getCountry($query->countryid);
            })
            ->addColumn('state', function($query){
                if($query->stateid == 0) {
                    return "All";
                } else {
                    return $query->getState($query->stateid);
                }
            })
            ...
            ->addColumn('action', function($query){
                $editx = "<a href='".route('admin.shipping.edit', $query->id)."' class='btn btn-primary'><i class='far fa-edit'></i></a>";
                $deletex = "<a href='".route('admin.shipping.destroy', $query->id)."' class='btn btn-danger ml-2 delete_item'><i class='far fa-trash-alt'></i></a>";
                return $editx.$deletex;
            })
            ->rawColumns(['country', 'state', 'charge', 'status', 'action'])
            ->setRowId('id');
    }
